...
namespace LR_7
{
    public partial class Main : Form
    {
        ...
        private void deleteButton_Click(object sender, EventArgs e)
        {
            ...
            {
                var confirmation = MessageBox.Show("Вы уверена, что хотите удалить выбранные вектора?", "Suppression", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (confirmation == DialogResult.Yes)
                {
                    for (int i = vectorView.SelectedItems.Count - 1; i >= 0; i--)
                    {
                        ListViewItem itm = vectorView.SelectedItems[i];
                        vectorView.Items[itm.Index].Remove();
                        vectors.RemoveAt(itm.Index);
                    }
                }
            }
            else
        ...
        {
            string path1 = @"C:\Users\veron\source\repos\csharp\Console\ЛР07\VectorsSymbols.txt";
        ...
        {
            Edit edit = new Edit(ref vectors[vectorView.SelectedItems[0].Index]);
            edit.Show();
        }
    }
}
